...
public class JdbcTarefaDao {
	...
	public void adiciona(Tarefa tarefa) {
		String sql = "insert into atrefas (descricao, finalizado) values (?,?)";
	...
	public void remove(Tarefa tarefa) {
		...
		String sql = "delete from atrefas where id = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
	...
	public void altera(Tarefa tarefa) {
		String sql = "update atrefas set descricao = ?, finalizado = ?, dataFinalizacao = ? where id = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, tarefa.getDescricao());
	...
	public List<Tarefa> lista() {
		try {
			...
			PreparedStatement stmt = this.connection
					.prepareStatement("select * from atrefas");
	...
	public Tarefa buscaPorId(Long id) {
		...
		try {
			PreparedStatement stmt = this.connection
					.prepareStatement("select * from atrefas where id = ?");
			stmt.setLong(1, id);
	...
	public void finaliza(Long id) {
		...
		String sql = "update atrefas set finalizado = ?, dataFinalizacao = ? where id = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
